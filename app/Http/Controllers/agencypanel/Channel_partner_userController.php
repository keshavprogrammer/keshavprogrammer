...
class Channel_partner_userController extends Controller
{
    ...
    public function manage() {
    	...
    	$data = Channel_partner_user::where("agenc_id", "=", Auth::id())->paginate($this->pagination);
        return view('agencypanel.managechannel_partner_user', compact('data'));
    }
    public function channelusersinglesms(Request $request) {
        $input = $request->all();
        ...
        $agentData = Channel_partner_user::query()->where('id', '=', $agntId)->get();
        ...
        if($sentResponse->sid != ''){
            ...
            return redirect()->route('agencypanel.channel_partner_user.manage')->with('success', 'SMS sent successfully.');
        }
        else{
            return redirect()->route('agencypanel.channel_partner_user.manage')->withErrors(['SMS is not sent']);
        }
    }
    ...
    public function add() {
        $channel_partner = Channel_partner::where("agenc_id", "=", Auth::id())->where("status", "=", "1")->pluck('name', 'id');
        $data = array('type'=>'add','agenc_id'=>Auth::id(), 'channel_partner'=>$channel_partner);        
        return view('agencypanel.addchannel_partner_user', compact('data'));
    }
    ...
    public function save(Request $request) {
        ...
        if ($validator->fails()) { 
            $channel_partner = Channel_partner::where("agenc_id", "=", Auth::id())->where("status", "=", "1")->pluck('name', 'id');
            $data = array('type'=>'add', 'input'=>$input, 'agenc_id'=>Auth::id(), 'channel_partner'=>$channel_partner, 'error'=>$validator->messages());
            return view('agencypanel.addchannel_partner_user', compact('data'));
            exit();            
        }
        ...
        if(isset($input["w9_file"])) {
            ...
            $upload_dir_path = public_path()."/uploads/channel_partner_user_files";
        ...
        if($agency->id>0) {
            return redirect()->route('agencypanel.channel_partner_user.manage')->with('success', 'Created successfully.');
        } else {
            return redirect()->route('agencypanel.channel_partner_user.add')->withErrors(['Error creating record. Please try again.']);
        }
    }
    ...
    public function edit($id) {
        ...
        $data = array('type'=>'edit', 'input'=>$input, 'agenc_id'=>Auth::id(), 'channel_partner'=>$channel_partner);
	    return view('agencypanel.addchannel_partner_user', compact('data'));
	}
	...
	public function update(Request $request) {
        ...
        if ($validator->fails()) { 
            $channel_partner = Channel_partner::where("agenc_id", "=", Auth::id())->where("status", "=", "1")->pluck('name', 'id');
            $data = array('type'=>'Edit', 'input'=>$input, 'agenc_id'=>Auth::id(), 'channel_partner'=>$channel_partner, 'error'=>$validator->messages());
            return view('agencypanel.addchannel_partner_user', compact('data'));
            exit();            
        }
        $update = array();
        ...
        return redirect()->route('agencypanel.channel_partner_user.manage')->with('success', 'Updated successfully.');
	...
	public function delete($id) {
        ...
        return redirect()->route('agencypanel.channel_partner_user.manage')->with('success', 'Deleted successfully.');
    }
