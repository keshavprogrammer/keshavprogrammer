...
class Channel_partnerController extends Controller
{
    ...
    public function channelsinglesms(Request $request) {
        $input = $request->all();
        ...
        $agentData = Channel_partner::query()->where('id', '=', $agntId)->get();
        ...
        if($sentResponse->sid != ''){
            ...
            return redirect()->route('agencypanel.channel_partner.manage')->with('success', 'SMS sent successfully.');
        }
        else{
            return redirect()->route('agencypanel.channel_partner.manage')->withErrors(['SMS is not sent']);
        }
    }
    ...
    public function save(Request $request) {
        ...
        if(isset($input["w9_file"])) {
            ...
            $upload_dir_path = public_path()."/uploads/channel_partner_files";
